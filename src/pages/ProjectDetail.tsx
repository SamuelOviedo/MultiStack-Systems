...
export default function ProjectDetail() {
  ...
  return (
    <div className="min-h-screen bg-background">
          ...
          <div className="space-y-6">
              ...
              <div className="flex items-start justify-between mb-4">
                <div className="flex-1">
                  ...
                  {editing ? (
                    ...
                    <span className={`inline-block font-display text-[10px] px-2 py-0.5 rounded border ${cfg.color}`}>{cfg.label}</span>
                  )}
                </div>
                <div className="flex gap-2 ml-4">
                  {editing ? (
                    <>
                      ...
                      <Button onClick={() => setEditing(false)} variant="outline" size="sm" className="font-display text-[10px]">
              ...
              <div className="border-t border-border pt-4 space-y-2">
                ...
                {editing ? (
                  <div className="space-y-2">
                    {[["client_name","Nombre"],["client_email","Email"],["client_phone","Teléfono"]].map(([k,label]) => (
            ...
            <Tabs defaultValue="servicios">
              <TabsList className="bg-card border border-border w-full justify-start h-auto p-1 gap-1">
                <TabsTrigger value="servicios" className="font-display text-xs data-[state=active]:bg-primary/10 data-[state=active]:text-primary">
                  Servicios
                </TabsTrigger>
                <TabsTrigger value="tickets" className="font-display text-xs data-[state=active]:bg-primary/10 data-[state=active]:text-primary">
                  Tickets {openCount > 0 && (
                    <span className="ml-1 bg-accent/20 text-accent font-display text-[10px] px-1.5 rounded-full">{openCount}</span>
                  )}
                </TabsTrigger>
                <TabsTrigger value="acceso" className="font-display text-xs data-[state=active]:bg-primary/10 data-[state=active]:text-primary">
                  <Link2 className="h-3 w-3 mr-1" />Acceso
                </TabsTrigger>
              </TabsList>

              {/* ── Tab: Servicios ── */}
              <TabsContent value="servicios" className="mt-4 space-y-6">
                <div className="rounded-lg border border-border bg-card p-6">
                  ...
                  {services.length === 0 ? (
                    <p className="text-xs text-muted-foreground text-center py-4 border border-dashed border-border rounded-lg">Sin servicios registrados.</p>
                  ) : (
                    <div className="space-y-2">
                      {services.map(s => {
                        ...
                        return (
                          <div key={s.id} className="flex items-center justify-between rounded-md border border-border px-3 py-2.5">
                            ...
                            <div className="flex items-center gap-3">
                              {days !== null && (
                                <span className={`font-display text-[10px] ${days <= 7 ? "text-red-400" : days <= 30 ? "text-yellow-400" : "text-muted-foreground"}`}>{days}d</span>
                              )}
                              {s.cost_yearly && (
                                <span className="font-display text-[10px] text-primary">{s.cost_yearly} {s.currency}/año</span>
                              )}
                              <button onClick={async () => { await deleteService(s.id); load(); }}
              ...
              <TabsContent value="tickets" className="mt-4">
                <div className="rounded-lg border border-border bg-card p-6">
                  ...
                  {filteredTickets.length === 0 ? (
                    <p className="text-xs text-muted-foreground text-center py-8 border border-dashed border-border rounded-lg">Sin tickets.</p>
                  ) : (
                    <div className="space-y-2">
                        ...
                        return (
                          <button key={t.id} onClick={() => setSelectedTicket(t)}
                            ...
                            <div className="flex-1 min-w-0">
                              ...
                              <p className="text-[10px] text-muted-foreground mt-0.5">{TICKET_TYPE_LABELS[t.type]} · {new Date(t.created_at).toLocaleDateString("es-HN")}</p>
                            </div>
                            <div className="flex items-center gap-2 shrink-0">
                              <span className={`font-display text-[10px] px-1.5 py-0.5 rounded border ${pc.color} ${pc.pulse ? "animate-pulse" : ""}`}>{pc.label}</span>
                              <span className={`font-display text-[10px] px-1.5 py-0.5 rounded border ${sc.color}`}>{sc.label}</span>
                              {t.message_count > 0 && (
              ...
              <TabsContent value="acceso" className="mt-4">
                <div className="rounded-lg border border-border bg-card p-6">
                  <ClientAccessSection projectId={project.id} tokens={tokens} onRefresh={load} />
                </div>
              </TabsContent>
            </Tabs>
          </div>
                ...
                return (
                  <div key={stage.key}>
                    <button onClick={() => setActiveStage(stage)}
                      className={`w-full flex items-start gap-3 rounded-lg px-3 py-3 text-left transition-colors ${done ? "hover:bg-primary/5 border border-primary/20" : "hover:bg-muted/20 border border-transparent"}`}>
                      <div className="mt-0.5 shrink-0">
                        {done ? <CheckCircle2 className="h-4 w-4 text-primary" /> : <Circle className="h-4 w-4 text-border" />}
                      </div>
                      <div className="flex-1 min-w-0">
                        <p className={`font-display text-xs font-medium ${done ? "text-foreground" : "text-muted-foreground"}`}>{stage.label}</p>
                        {done && stageData ? (
                          <>
                    ...
                    {i < PIPELINE_STAGES.length - 1 && <div className="ml-5 w-px h-3 bg-border" />}
                  </div>
                );
              })}
            </div>
      ...
      {activeStage && (
        <StageDrawer projectId={project.id} stage={activeStage}
          existing={stages.find(s => s.stage_key === activeStage.key) ?? null}
          open={!!activeStage} onClose={() => setActiveStage(null)} onSaved={load} />
      )}
      <ServiceForm projectId={project.id} open={showServiceForm}
        onClose={() => setShowServiceForm(false)} onSaved={load} />
      <TicketDrawer ticket={selectedTicket} open={!!selectedTicket}
        onClose={() => setSelectedTicket(null)} onUpdated={load}
