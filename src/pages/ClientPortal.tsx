      ...
      <div className="bg-card border border-border rounded-xl w-full max-w-md space-y-4 p-6">
        ...
        <div className="grid grid-cols-2 gap-3">
          <div className="space-y-1">
            <label className="font-mono text-[10px] text-muted-foreground">nombre*</label>
        ...
        <div className="space-y-1">
          <label className="font-mono text-[10px] text-muted-foreground">tipo</label>
        ...
        <div className="space-y-1">
          <label className="font-mono text-[10px] text-muted-foreground">título*</label>
          <Input value={title} onChange={e => setTitle(e.target.value)} placeholder="Resumen breve del problema" className={inputClass} />
        </div>

        <div className="space-y-1">
          <label className="font-mono text-[10px] text-muted-foreground">descripción</label>
          <Textarea value={description} onChange={e => setDescription(e.target.value)}
            placeholder="Detalla el problema o solicitud..."
            className="bg-background border-border font-mono text-xs resize-none" rows={4} />
        </div>
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="space-y-3">
        {messages.length === 0 ? (
          <p className="text-xs text-muted-foreground text-center py-6 font-mono">sin mensajes todavía.</p>
        ) : messages.map(m => {
      ...
      {ticket.status !== "cerrado" && ticket.status !== "resuelto" && (
        <div className="rounded-lg border border-border bg-card p-4 space-y-3">
          <p className="font-mono text-[10px] text-muted-foreground">responder</p>
          <Input value={name} onChange={e => setName(e.target.value)}
            placeholder="Tu nombre" className="bg-background border-border font-mono text-xs h-8" />
          <Textarea value={reply} onChange={e => setReply(e.target.value)}
            placeholder="Escribe tu mensaje..."
            className="bg-background border-border font-mono text-xs resize-none" rows={3}
  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-background flex items-center justify-center">
        <p className="font-mono text-muted-foreground text-sm animate-pulse">verificando acceso...</p>
      </div>
    );
  }
    ...
    return (
      <div className="min-h-screen bg-background flex items-center justify-center p-6">
        <div className="text-center space-y-2">
          ...
          <p className="font-mono text-xs text-muted-foreground">Este enlace no es válido o ha expirado.</p>
        </div>
      </div>
    );
  }
      ...
      <div className="max-w-2xl mx-auto px-6 py-8 space-y-6">
        ...
        <div className="rounded-xl border border-border bg-card p-5 space-y-2">
          ...
          <div className="flex items-center gap-2 pt-1">
            <span className="font-mono text-[10px] px-2 py-0.5 rounded border border-primary/30 text-primary bg-primary/10">
              {STATUS_MAP[project.estado] ?? project.estado}
            </span>
            {project.client_name && (
              <span className="font-mono text-[10px] text-muted-foreground">{project.client_name}</span>
            )}
          </div>
        </div>
          ...
          <div className="space-y-4">
            ...
            {tickets.length === 0 ? (
              <div className="rounded-lg border border-border bg-card p-6 text-center">
                <p className="text-xs text-muted-foreground font-mono">sin tickets todavía.</p>
                  ...
                  return (
                    ...
                    >
                      <div className="flex items-start justify-between gap-2">
                        ...
                        <span className={`font-mono text-[10px] px-2 py-0.5 rounded border shrink-0 ${priorityCfg.color} ${priorityCfg.pulse ? "animate-pulse" : ""}`}>
                          {priorityCfg.label}
                        </span>
                      </div>
                      <div className="flex items-center gap-2">
                        ...
                        <span className={`font-mono text-[10px] px-1.5 py-0.5 rounded border ${statusCfg.color}`}>
                          {statusCfg.label}
                        </span>
                        {t.message_count > 0 && (
